    ...
    serviceCommand
        ...
        .action(async (options) => {
            try {
                ...
                const response = await askService(signal, options.conversation, options.prompt);
                console.log('✅ 입력 전송에 성공했습니다.');
                console.log(response);
                process.exit(0);
            } catch (error) {
                console.error('❌ 질문 전송 중 오류 발생:', error instanceof Error ? error.message : String(error));
                process.exit(1);
            }
        });
    ...
    serviceCommand
        ...
        .action(async () => {
            try {
                ...
                console.error('❌ 대화 목록 가져오기 중 오류 발생:', error instanceof Error ? error.message : String(error));
                process.exit(1);
            }
        });
    ...
    serviceCommand
        ...
        .action(async (options) => {
            try {
                ...
                const response = await getResponseService(signal, options.conversation);
                console.log(response);
                process.exit(0);
            } catch (error) {
                console.error('❌ 응답 가져오기 중 오류 발생:', error instanceof Error ? error.message : String(error));
